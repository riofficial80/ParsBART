...
def token_infilling(
    ...
    **This function operates on tokenized text.**
    ...
    if span_length > 0:
        ...
        tokenized_sequence_numpy = tokenized_sequence.numpy()
        # indices = np.argwhere(tokenized_sequence_numpy==1) # finding the positions of pad tokens
        # tokenized_sequence_numpy = np.delete(tokenized_sequence_numpy, indices) # tokenized sequence without pad token
        mask_insertion_position = random.randint(0, tokenized_sequence_numpy.shape[0])
        tokenized_sequence_numpy = np.insert(tokenized_sequence_numpy, mask_insertion_position, 4)
        perturbed_ids = torch.from_numpy(tokenized_sequence_numpy) # if the span length is 0, the text is not perturbed
